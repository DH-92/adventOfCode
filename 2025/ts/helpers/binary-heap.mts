 ...
 */
function siftDown<T>(compare: Function, heap: Array<T>, startIndex: number, i: number) {
  var item = heap[i],
    parentIndex,
    parent
 ...
 * @param {number}   i       - Index.
 */
function siftUp<T>(compare: Function, heap: Array<T>, i: number) {
 ...
 */
function push<T>(compare: Function, heap: Array<T>, item: T) {
  heap.push(item)
 ...
 * @return {any}
 */
function pop<T>(compare: Function, heap: Array<T>): T {
 ...
 * @return {any}
 */
function replace<T>(compare: Function, heap: Array<T>, item: T): T {
 ...
 * @return {any}
 */
function pushpop<T>(compare: Function, heap: Array<T>, item: T): T {
  var tmp
 ...
 */
function heapify<T>(compare: Function, array: Array<T>) {
  var n = array.length,
    l = n >> 1,
    i = l
 ...
 * @return {array}
 */
function consume<T>(compare: Function, heap: Array<T>): Array<T> {
  var l = heap.length,
    i = 0
 ...
 * @param {array}
 */
function nsmallest<T>(compare: Function, n: number, iterable: T) {
  ...
  if (iterables.isArrayLike(iterable)) {
    ...
    for (i = n, l = iterable.length; i < l; i++)
      if (reverseCompare(iterable[i], result[0]) > 0) replace(reverseCompare, result, iterable[i])
 ...
 * @param {array}
 */
function nlargest(compare: Function, n: number, iterable: any) {
  if (arguments.length === 2) {
    iterable = n
    n = compare
    compare = DEFAULT_COMPARATOR
  ...
  if (iterables.isArrayLike(iterable)) {
    ...
    for (i = n, l = iterable.length; i < l; i++)
      if (compare(iterable[i], result[0]) > 0) replace(compare, result, iterable[i])
...
export class Heap<T> {
  ...
  //   // If iterable is an array, we can be clever about it
  //   if (iterables.isArrayLike(iterable)) items = iterable.slice()
  //   else items = iterables.toArray(iterable)
